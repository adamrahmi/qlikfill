...
// Define your initial default text here
const initialCustomizedText = {
  "1. Not product": {
  ...
  "3. Resolution": {
      "Survey": "Glad to hear this works now!",
      "Conclusion": "Based on the evidence so far",
  },
};

// Load customized text and original text from Chrome storage
chrome.storage.sync.get(["customizedtext", "originaltext"], function (result) {
  ...
  // Enforce text values for default and original text
  enforceTextValues(customizedText);
  enforceTextValues(originalText);

  // If default values are not yet set, initialize them
  if (Object.keys(customizedText).length === 0) {
      // Set initial default values in Chrome storage
      chrome.storage.sync.set({ customizedtext: initialCustomizedText });

      // Set original values in Chrome storage (unchanged by the user)
      chrome.storage.sync.set({ originaltext: initialCustomizedText });
  }
});
